...
请确保在使用前配置火山引擎API的鉴权：
```bash
export VOLCENGINE_API_KEY="您的API_KEY"
```
也可以创建一个 `.env` 文件来管理环境变量（如果使用 `python-dotenv`）。
...
始终使用 `--archive-dir /Users/wanglingwei/Movies/violinvault/SynologyDrive/Clipping/outputs/reports` 和 `--title` 参数。
